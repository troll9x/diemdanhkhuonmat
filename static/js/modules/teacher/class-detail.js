...
let classData    = null;
let studentsFull = [];   // full list for search filtering
...
function renderSchedules(schedules) {
    ...
    const weekMap = new Array(7).fill(null);
    schedules.forEach(s => { weekMap[s.day_of_week] = s; });
...
async function loadStudents() {
    ...
    try {
        ...
        // Update session count stat from sessions API (best effort)
        try {
...
async function saveSched() {
    ...
    try {
        ...
        loadSchedList();
        loadClassDetail();  // refresh inline schedule display
    } catch (e) {
        setAlert(alertBox, e.message, 'danger');
    }
}
