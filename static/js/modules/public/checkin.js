/**
 * Module Điểm Danh Công Khai (Public Student Check-in)
 * Trang điểm danh không yêu cầu đăng nhập — sinh viên nhập mã số SV và chụp ảnh khuôn mặt.
 * Được truy cập qua QR Code do giảng viên/admin tạo, URL chứa session_id.
 * Luồng xử lý:
 *   1. Tải thông tin phiên học theo session_id trong URL
 *   2. Sinh viên nhập mã số sinh viên
 *   3. Bật camera → chụp ảnh JPEG
 *   4. Gửi ảnh + mã SV + session_id lên POST /api/checkin
 *   5. Xử lý kết quả: thành công / đã điểm danh / giả mạo / không nhận ra...
 *
 * Hàm chính:
 *   loadSessionInfo(id)     — Tải và hiển thị thông tin phiên học (lớp, môn, ngày giờ, trạng thái)
 *   startCamera()           — Bật webcam front-facing, tạo canvas capture
 *   stopCamera()            — Tắt webcam và giải phóng stream
 *   captureAndSubmit()      — Chụp frame từ video → gửi FormData lên API checkin
 *   handleCheckinResult()   — Xử lý và hiển thị kết quả từ API
 *   showResult(msg,type)    — Hiển thị hộp kết quả (success/error/info)
 *   resetForm()             — Reset form và dừng camera để thử lại
 *   disableForm()           — Vô hiệu form sau khi điểm danh thành công
 */

// ID phiên học — lấy từ URL hoặc biến toàn cục của trang
let sessionId = null;

// MediaStream từ webcam
let stream = null;

// Canvas dùng để chụp frame từ video
let canvas = null;
