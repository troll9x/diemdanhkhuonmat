/**
 * Module Quản Lý Thời Khóa Biểu (Admin Class Schedules)
 * CRUD lịch học theo ngày trong tuần: thêm/sửa/xóa buổi học, gắn với lớp học.
 * Hàm chính:
 *   loadClassrooms()        — Tải danh sách lớp học vào dropdown
 *   loadSchedules(page)     — Tải và phân trang danh sách lịch học
 *   renderSchedulesTable()  — Render bảng lịch học (lớp, thứ, giờ, ngưỡng muộn)
 *   renderPagination()      — Render điều hướng phân trang
 *   editSchedule(id)        — Mở modal sửa lịch học
 *   saveSchedule(event)     — Lưu lịch học (POST hoặc PUT)
 *   deleteSchedule(id)      — Xóa lịch học sau khi xác nhận
 *   escapeHtml()            — Escape HTML ngăn XSS
 */
let currentPage = 1;
const itemsPerPage = 10;

// Ánh xạ day_of_week (1–7) sang tên thứ tiếng Việt
const dayNames = {
    1: 'Thứ 2',
    2: 'Thứ 3',
    3: 'Thứ 4',
