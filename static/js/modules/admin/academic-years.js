/**
 * Module Quản Lý Năm Học (Admin Academic Years)
 * CRUD năm học: tạo/sửa/xóa năm học, đặt năm học hiện tại.
 * Hàm chính:
 *   loadAcademicYears(page) — Tải và phân trang danh sách năm học
 *   renderTable(items)      — Render bảng năm học (tên, ngày bắt đầu/kết thúc, hiện tại, trạng thái)
 *   renderPagination()      — Render điều hướng phân trang
 *   openModal(item)         — Mở modal tạo mới hoặc chỉnh sửa năm học
 *   saveItem(event)         — Lưu năm học (POST hoặc PUT)
 *   deleteItem(id)          — Xóa năm học sau khi xác nhận
