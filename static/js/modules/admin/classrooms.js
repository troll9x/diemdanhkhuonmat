/**
 * Module Quản Lý Lớp Học (Admin Classrooms)
 * CRUD lớp học: tạo/sửa/xóa lớp, phân công giảng viên, quản lý sinh viên và môn học trong lớp.
 * Hàm chính:
 *   loadLecturers()          — Tải danh sách giảng viên vào dropdown
 *   loadSubjectOptions()     — Tải danh sách môn học vào dropdown
 *   loadClassrooms(page)     — Tải và phân trang danh sách lớp (lọc theo tên/giảng viên/trạng thái)
 *   renderClassroomsTable()  — Render bảng lớp học
 *   renderPagination()       — Render điều hướng phân trang
 *   editClassroom(id)        — Mở modal sửa lớp
 *   saveClassroom(event)     — Lưu thông tin lớp (POST hoặc PUT)
 *   deleteClassroom(id)      — Xóa lớp sau khi xác nhận
 *   openStudentsPanel()      — Mở modal quản lý sinh viên của lớp
 *   loadEnrolledStudents()   — Tải danh sách sinh viên đã đăng ký
 *   smSearchStudents()       — Tìm kiếm sinh viên để thêm vào lớp
 *   addStudentToClass()      — Thêm sinh viên vào lớp
 *   removeStudentFromClass() — Xóa sinh viên khỏi lớp
 *   openSubjectsPanel()      — Mở modal quản lý môn học của lớp
 *   loadClassSubjects()      — Tải danh sách môn học đã gán cho lớp
 *   addSubjectToClass()      — Thêm môn học vào lớp
 *   removeSubjectFromClass() — Xóa môn học khỏi lớp
