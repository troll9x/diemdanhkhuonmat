/**
 * Module Quản Lý Sinh Viên (Admin Students)
 * CRUD sinh viên: tạo/sửa/xóa tài khoản sinh viên, lọc theo khoa/chuyên ngành/trạng thái.
 * Hàm chính:
 *   loadDepartments()       — Tải danh sách khoa vào dropdown form và bộ lọc
 *   loadMajors()            — Tải danh sách chuyên ngành (phụ thuộc khoa đã chọn)
 *   loadStudents(page)      — Tải và phân trang danh sách sinh viên
 *   renderStudentsTable()   — Render bảng sinh viên với nút sửa/xóa/reset khuôn mặt
 *   renderPagination()      — Render điều hướng phân trang
 *   resetStudentForm()      — Reset form về trạng thái thêm mới
 *   editStudent(id)         — Mở modal sửa thông tin sinh viên
 *   saveStudent(event)      — Lưu sinh viên (POST hoặc PUT); mật khẩu tùy chọn khi sửa
 *   deleteStudent(id)       — Xóa sinh viên sau khi xác nhận
 *   resetFaceData(id)       — Xóa toàn bộ dữ liệu khuôn mặt (sinh viên phải đăng ký lại)
