/**
 * Module Quản Lý Phiên Học (Admin Class Sessions)
 * CRUD phiên học (buổi dạy cụ thể): tạo/sửa/xóa/hủy phiên học trong ngày.
 * Mỗi phiên có: lớp học, môn học, phòng học, giảng viên, ngày, giờ bắt đầu/kết thúc, trạng thái.
 * Hàm chính:
 *   loadLookups()           — Tải song song dropdown lớp học/môn học/phòng/giảng viên
 *   loadClassrooms()        — Tải dropdown lớp học
 *   loadSubjects()          — Tải dropdown môn học
 *   loadRooms()             — Tải dropdown phòng học
 *   loadLecturers()         — Tải dropdown giảng viên
 *   loadSessions(page)      — Tải và phân trang danh sách phiên học
 *   renderSessionsTable()   — Render bảng phiên học với màu trạng thái
 *   editSession(id)         — Mở modal sửa phiên học
 *   saveSession(event)      — Lưu phiên học (POST hoặc PUT)
 *   deleteSession(id)       — Xóa phiên học sau khi xác nhận
 *   escapeHtml()            — Escape HTML ngăn XSS
 */

// Nhãn hiển thị cho từng trạng thái phiên học
const statusLabels = {
    scheduled: 'Lên lịch',
...
// Màu Bootstrap badge cho từng trạng thái
const statusColors = {
    scheduled: 'secondary',
    ongoing:   'warning',
    completed: 'success',
