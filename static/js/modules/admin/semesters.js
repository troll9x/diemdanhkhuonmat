/**
 * Module Quản Lý Học Kỳ (Admin Semesters)
 * CRUD học kỳ: tạo/sửa/xóa học kỳ, liên kết với năm học.
 * Hàm chính:
 *   loadAcademicYears()     — Tải dropdown năm học cho bộ lọc và form
 *   loadSemesters(page)     — Tải và phân trang danh sách học kỳ (lọc theo năm học/trạng thái)
 *   renderTable(items)      — Render bảng học kỳ (tên, năm học, ngày, học kỳ hiện tại, trạng thái)
 *   renderPagination()      — Render điều hướng phân trang
 *   openModal(item)         — Mở modal tạo mới hoặc chỉnh sửa học kỳ
 *   saveItem(event)         — Lưu học kỳ (POST hoặc PUT)
 *   deleteItem(id)          — Xóa học kỳ sau khi xác nhận
 *   showState(s)            — Chuyển trạng thái UI: loading / empty / table
 *   showAlert(msg,type)     — Hiện Bootstrap alert tự đóng sau 4 giây
