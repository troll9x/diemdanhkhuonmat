/**
 * Module Quản Lý Giảng Viên (Admin Lecturers)
 * CRUD giảng viên: tạo/sửa/xóa tài khoản giảng viên, lọc theo khoa/trạng thái.
 * Hàm chính:
 *   loadDepartments()       — Tải danh sách khoa vào dropdown form và bộ lọc
 *   loadLecturers(page)     — Tải và phân trang danh sách giảng viên
 *   renderLecturersTable()  — Render bảng giảng viên với nút sửa/xóa
 *   renderPagination()      — Render điều hướng phân trang
 *   resetLecturerForm()     — Reset form về trạng thái thêm mới
 *   editLecturer(id)        — Mở modal sửa thông tin giảng viên
 *   saveLecturer(event)     — Lưu giảng viên (POST hoặc PUT)
 *   deleteLecturer(id)      — Xóa giảng viên sau khi xác nhận
