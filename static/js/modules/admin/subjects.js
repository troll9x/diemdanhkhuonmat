/**
 * Module Quản Lý Môn Học (Admin Subjects)
 * CRUD môn học: tạo/sửa/xóa môn, gắn với khoa, lọc theo tên/khoa/trạng thái.
 * Hàm chính:
 *   loadDepartments()       — Tải danh sách khoa vào dropdown form và bộ lọc
 *   loadSubjects(page)      — Tải và phân trang danh sách môn học
 *   renderSubjectsTable()   — Render bảng môn học (mã, tên, khoa, tín chỉ, trạng thái)
 *   renderPagination()      — Render điều hướng phân trang
 *   editSubject(id)         — Mở modal sửa môn học
 *   saveSubject(event)      — Lưu môn học (POST hoặc PUT)
 *   deleteSubject(id)       — Xóa môn học sau khi xác nhận
