"""
Routes Đăng Ký Khuôn Mặt - Legacy (Legacy Face Registration Routes)
Blueprint dành cho luồng đăng ký khuôn mặt cũ (dùng model User + FaceCapture).
Luồng đăng ký mới dùng AppUser + AppFaceEmbedding (trong routes/student.py).

Endpoints chính:
  POST /api/users/face/register  — Upload và lưu ảnh khuôn mặt (FaceCapture)
                                   Dùng YOLO để phát hiện khuôn mặt, InsightFace để trích xuất embedding
  POST /api/users/face/train     — Kích hoạt huấn luyện lại mô hình từ tất cả FaceCapture trong DB
  GET  /api/users/face/status    — Kiểm tra trạng thái đăng ký khuôn mặt của user hiện tại
"""
import pickle
import uuid
