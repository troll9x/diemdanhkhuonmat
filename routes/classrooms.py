"""
Routes Quản Lý Lớp Học (Classroom Management)
Blueprint: classrooms_bp — tiền tố URL: /api/classrooms

Endpoints (tất cả yêu cầu JWT; tạo/sửa/xóa chỉ dành cho admin hoặc giảng viên):
  GET    /                      — Danh sách lớp có phân trang, lọc theo tên/giảng viên/trạng thái
  POST   /                      — Tạo lớp học mới
  GET    /<id>                  — Chi tiết lớp (kèm sinh viên, môn học, lịch học)
  PUT    /<id>                  — Cập nhật thông tin lớp
  DELETE /<id>                  — Xóa lớp (chỉ xóa được nếu chưa có điểm danh)
  GET    /<id>/students         — Danh sách sinh viên đăng ký lớp
  POST   /<id>/students         — Thêm danh sách sinh viên vào lớp
  DELETE /<id>/students/<sid>   — Xóa một sinh viên khỏi lớp
  GET    /<id>/subjects         — Danh sách môn học của lớp
  POST   /<id>/subjects         — Gán môn học cho lớp
  DELETE /<id>/subjects/<subid> — Xóa môn học khỏi lớp
"""
