"""
Routes Quản Lý Học Kỳ (Semester Management)
Blueprint: semesters_bp — tiền tố URL: /api/semesters

Học kỳ thuộc năm học (AcademicYear). Mỗi lớp học có thể gắn với một học kỳ.

Endpoints (tất cả yêu cầu JWT; tạo/sửa/xóa chỉ dành cho admin):
  GET    /      — Danh sách học kỳ có phân trang, lọc theo tên/năm học/trạng thái
  POST   /      — Tạo học kỳ mới
  GET    /<id>  — Chi tiết học kỳ
  PUT    /<id>  — Cập nhật học kỳ
  DELETE /<id>  — Xóa học kỳ (không xóa nếu có lớp học liên kết)
"""
