"""
Routes Quản Lý Lịch Học (Class Schedule Management)
Blueprint: class_schedules_bp — tiền tố URL: /api/class-schedules

Lịch học định nghĩa buổi học lặp lại hàng tuần (thứ, giờ bắt đầu/kết thúc, phòng).

Endpoints (tất cả yêu cầu JWT):
  GET    /      — Danh sách lịch học có phân trang, lọc theo lớp/thứ/phòng
  POST   /      — Tạo lịch học mới
  GET    /<id>  — Chi tiết lịch học
  PUT    /<id>  — Cập nhật lịch học
