"""
Routes Quản Lý Tòa Nhà (Building Management)
Blueprint: buildings_bp — tiền tố URL: /api/buildings

Tòa nhà thuộc Cơ sở (Campus). Chứa các phòng học (Room).

Endpoints (tất cả yêu cầu JWT; tạo/sửa/xóa chỉ dành cho admin):
  GET    /      — Danh sách tòa nhà có phân trang, lọc theo tên/cơ sở/trạng thái
  POST   /      — Tạo tòa nhà mới
  GET    /<id>  — Chi tiết tòa nhà (kèm danh sách phòng)
  PUT    /<id>  — Cập nhật tòa nhà
  DELETE /<id>  — Xóa tòa nhà (không xóa nếu còn phòng liên kết)
"""
