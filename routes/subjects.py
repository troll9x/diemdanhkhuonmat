"""
Routes Quản Lý Môn Học (Subject/Course Management)
Blueprint: subjects_bp — tiền tố URL: /api/subjects

Endpoints (tất cả yêu cầu JWT; tạo/sửa/xóa chỉ dành cho admin):
  GET    /      — Danh sách môn học có phân trang, lọc theo tên/khoa/trạng thái
  POST   /      — Tạo môn học mới (admin only)
  GET    /<id>  — Chi tiết môn học
  PUT    /<id>  — Cập nhật môn học
  DELETE /<id>  — Xóa môn học (không xóa được nếu có lớp học đang sử dụng)
"""
