"""
Routes Quản Lý Cơ Sở (Campus Management)
Blueprint: campuses_bp — tiền tố URL: /api/campuses

Cơ sở là đơn vị địa lý cao nhất trong cấu trúc trường (ví dụ: Cơ sở 1, Cơ sở 2).
Mỗi cơ sở chứa nhiều Tòa nhà (Building).

Endpoints (tất cả yêu cầu JWT; tạo/sửa/xóa chỉ dành cho admin):
  GET    /      — Danh sách cơ sở có phân trang
  POST   /      — Tạo cơ sở mới
  GET    /<id>  — Chi tiết cơ sở
  PUT    /<id>  — Cập nhật cơ sở
  DELETE /<id>  — Xóa cơ sở (không xóa nếu còn tòa nhà liên kết)
"""
