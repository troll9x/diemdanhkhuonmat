"""
Routes Điểm Danh - Legacy (Legacy Attendance Routes)
Blueprint dành cho hệ thống điểm danh cũ (dùng model Attendance + User).
Các route mới hơn sử dụng AppAttendanceRecord + AppUser (trong routes/student.py, routes/teacher.py).

Endpoints chính:
  POST /api/attendance/checkin        — Điểm danh bằng khuôn mặt (legacy pipeline)
  GET  /api/attendance/report         — Báo cáo điểm danh theo lớp/ngày
  GET  /api/attendance/export         — Xuất báo cáo Excel (send_file với pandas)
  GET  /api/attendance/student/<id>   — Lịch sử điểm danh của một sinh viên
"""
import io
import pickle
