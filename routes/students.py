"""
Routes Quản Lý Sinh Viên - Legacy (Student Management)
Blueprint: students_bp — tiền tố URL: /api/students

Quản lý tài khoản sinh viên (model Student legacy, tách biệt với AppUser v2).

Endpoints (tất cả yêu cầu JWT; tạo/sửa/xóa chỉ dành cho admin):
  GET    /              — Danh sách sinh viên có phân trang, lọc nhiều chiều (khoa, trạng thái, khuôn mặt)
  POST   /              — Tạo tài khoản sinh viên mới (admin only)
  GET    /<id>          — Chi tiết sinh viên
  PUT    /<id>          — Cập nhật thông tin sinh viên
  DELETE /<id>          — Xóa sinh viên (soft delete)
  DELETE /<id>/face-data— Xóa toàn bộ dữ liệu khuôn mặt (FaceEmbedding) của sinh viên
"""
