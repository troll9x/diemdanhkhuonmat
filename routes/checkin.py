"""
Route Điểm Danh Công Khai (Public Check-in via QR Code)
Blueprint: checkin_bp — endpoint: POST /api/checkin

Endpoint công khai, KHÔNG yêu cầu JWT — sinh viên quét QR code và điểm danh mà không cần đăng nhập.

Endpoint:
  POST /api/checkin — Nhận multipart/form-data: image (ảnh khuôn mặt), session_id, student_code
                     Pipeline: xác minh session → xác minh student_code → nhận diện khuôn mặt
                               → kiểm tra gian lận → tạo bản ghi AttendanceRecord

Trạng thái trả về: success / already_checked_in / spoof / unknown / not_enrolled / session_closed
"""
