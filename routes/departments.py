"""
Routes Quản Lý Khoa (Department Management)
Blueprint: departments_bp — tiền tố URL: /api/departments

Endpoints (tất cả yêu cầu JWT; tạo/sửa/xóa chỉ dành cho admin):
  GET    /          — Danh sách khoa có phân trang, hỗ trợ tìm kiếm và lọc theo trạng thái
  POST   /          — Tạo khoa mới (admin only)
  GET    /<id>      — Chi tiết một khoa
  PUT    /<id>      — Cập nhật thông tin khoa (admin only)
  DELETE /<id>      — Xóa khoa (admin only); không xóa được nếu còn giảng viên/SV liên kết
"""
