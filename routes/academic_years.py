"""
Routes Quản Lý Năm Học (Academic Year Management)
Blueprint: academic_years_bp — tiền tố URL: /api/academic-years

Năm học là đơn vị thời gian cao nhất trong cấu trúc trường. Mỗi năm học có thể đặt làm "năm hiện tại".

Endpoints (tất cả yêu cầu JWT; tạo/sửa/xóa chỉ dành cho admin):
  GET    /      — Danh sách năm học có phân trang
  POST   /      — Tạo năm học mới
  GET    /<id>  — Chi tiết năm học
  PUT    /<id>  — Cập nhật năm học
  DELETE /<id>  — Xóa năm học (không xóa nếu có học kỳ liên kết)
"""
