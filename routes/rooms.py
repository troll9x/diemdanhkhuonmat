"""
Routes Quản Lý Phòng Học (Room Management)
Blueprint: rooms_bp — tiền tố URL: /api/rooms

Phòng học thuộc Tòa nhà (Building). Được gán cho lịch học và phiên học.

Endpoints (tất cả yêu cầu JWT; tạo/sửa/xóa chỉ dành cho admin):
  GET    /      — Danh sách phòng học có phân trang, lọc theo tên/tòa nhà/trạng thái
  POST   /      — Tạo phòng học mới
  GET    /<id>  — Chi tiết phòng học
  PUT    /<id>  — Cập nhật phòng học
  DELETE /<id>  — Xóa phòng học
"""
